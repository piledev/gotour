...
// Record returns URL
func (ur *URLRecorder) Record(url string) bool {
	ur.mu.Lock()
	defer ur.mu.Unlock()
	if ur.exist[url] {
		return true
...
var r URLRecorder = URLRecorder{exist: make(map[string]bool)}
